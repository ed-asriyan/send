...
async function sweepStale() {
  ...
  try {
    ...
    console.warn("OPFS sweepStale failed or OPFS unavailable, using memory fallback");
    useMemory = true;
  }
}
...
async function handleEncrypt(id: number, data: ArrayBuffer, fileName: string) {
  ...
  try {
    ...
    console.warn("OPFS upload failed, using memory", err);
    useMemory = true;
    memoryChunks.set(0, encData); // We just store the whole encrypted file at index 0 for upload
  }
...
function handleReadChunk(id: number, offset: number, size: number) {
  ...
  if (useMemory) {
    const encData = memoryChunks.get(0);
    if (!encData) {
      self.postMessage({id, type: 'error', message: 'No memory data for upload'});
      return;
    }
    buf.set(encData.subarray(offset, offset + size));
  } else {
    if (!uploadReadHandle) {
...
async function handleDecryptAndStore(
  ...
  if (!downloadWriteHandle) {
    try {
      ...
      console.warn("OPFS create handle failed, switching to memory storage", e);
      useMemory = true;
      memoryChunks.set(chunkNo, decrypted);
      self.postMessage({id, type: 'stored'})
      return;
...
initPromise.then(
  ...
  (err) => {
    ...
    self.postMessage({type: 'init_error', message: err ? err.toString() : 'Unknown error'});
  }
)
